...
def load_bird(path_to_dataset):
    rir_files = [file for file in Path(path_to_dataset).rglob("*.flac") if not file.name.startswith('._')]
...
def load_rir_dataset():
    train_files, valid_files, test_files = load_bird(
        "/home/cxw/GAN/fins dataset/BIRD")
    return train_files, valid_files, test_files


def load_speech_dataset():
    # speech_files = list(Path(
    #     "/home/cxw/GAN/fins dataset/DAPS").rglob("*.wav"))[:500]
    speech_files = [file for file in Path(
        "/home/cxw/GAN/fins dataset/DAPS").rglob("*.wav") if not file.name.startswith('._')]
